...
public class MenuDao extends BaseDao<Menu> {
	...
	public List<Menu> findAllActivitiList() {
		return find("from Menu where delFlag=:p1 and order by sort",new Parameter(Dict.DEL_FLAG_NORMAL));
	}
	...
	public List<Menu> findAllActivitiList(String userId) {
		return find("select distinct m from Menu m, Role r, User u where m in elements (r.menuList) and r in elements (u.roleList)" +
				" and m.delFlag=:p1 and r.delFlag=:p1 and u.delFlag=:p1 and u.id=:p3 order by m.sort", 
				new Parameter(Menu.DEL_FLAG_NORMAL,userId));
	}
